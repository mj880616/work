...
test('김포 공영화 투쟁 허브는 3개 대응영역과 행감·기자회견 하위페이지를 제공한다', async () => {
  ...
  expect(html).toContain('기자회견 준비페이지');
...
test('10/8 김포 기자회견 준비페이지는 기획안 편집·인쇄와 서버 체크리스트를 제공한다', async () => {
  const html = await read('p/gimpo-publicization-press-1008/index.html');
  expect(html).toContain('name="kptu-page-slug" content="gimpo-publicization-press-1008"');
