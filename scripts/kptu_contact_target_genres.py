...
OUT=ROOT/"contact-author-target-genres.json"
...
for row in data["items"]:
    if row.get("kind") not in TARGET: continue
    for c in row.get("contacts",[]):
        n=c.get("name")
        if not n or n in bad or not re.fullmatch(r"[가-힣]{2,4}",n):continue
        by.setdefault(n,{})[row["idx"]]={
            "idx":row["idx"],"date":row["date"],"kind":row["kind"],"title":row["title"],"url":row["url"],"role":c.get("role"),"snippet":c.get("snippet")
        }
out=[]
for n,docs in by.items():
    rows=sorted(docs.values(),key=lambda x:(x["date"],x["idx"]),reverse=True)
    if len(rows)>=2:out.append({"name":n,"count":len(rows),"documents":rows})
out.sort(key=lambda x:(-x["count"],x["name"]))
OUT.write_text(json.dumps({"candidates":out},ensure_ascii=False,indent=2),encoding="utf-8")
print(json.dumps([{"name":x["name"],"count":x["count"],"docs":[[r["date"],r["kind"],r["title"]] for r in x["documents"][:10]]} for x in out[:50]],ensure_ascii=False,indent=2))
