...
async function renderGrid(){
  ...
  grid.innerHTML=rows.length?rows.map(p=>{
    const ch=kids(p),m=p.metadata||{};
    return `<article class="ps3-project-card ps3-main-card" data-ps3-project="${p.id}" tabindex="0" role="button"><div class="ps3-card-top"><span class="ps3-kind main">${p.parent_id?'프로젝트':'상위 프로젝트'}</span>${p.status==='done'?'<span class="ps3-phase">완료</span>':''}</div><h3>${esc(p.name)}</h3><p>${esc(m.objective||p.description||'')}</p><div class="ps3-card-meta"><span>${esc(m.start_on||'시작일 미정')}</span><span>하위 ${ch.length}</span></div>${ch.length?`<div class="ps3-child-list">${ch.map(c=>`<button type="button" class="ps3-child-row" data-ps3-project="${c.id}"><span>↳</span><b>${esc(c.name)}</b><small>하위 프로젝트</small></button>`).join('')}</div>`:''}</article>`
...
async function fetchDetail(id){const p=projects.find(x=>x.id===id);if(!p)return null;const enc=encodeURIComponent(id);const [modules,workstreams,progress,milestones,tasks,docs,memos]=await Promise.all([api(`/rest/v1/app_project_modules?project_id=eq.${enc}&select=*&order=sort_order.asc`),api(`/rest/v1/app_project_workstreams?project_id=eq.${enc}&select=*&order=sort_order.asc`),api(`/rest/v1/app_project_progress_updates?project_id=eq.${enc}&select=*&order=effective_on.desc,created_at.desc`),api(`/rest/v1/app_project_milestones?project_id=eq.${enc}&select=*&order=start_at.asc.nullslast,sort_order.asc`),api(`/rest/v1/app_tasks?project_id=eq.${enc}&assignee_id=eq.${encodeURIComponent(user.id)}&select=*&order=due_at.asc.nullslast,created_at.desc`),api(`/rest/v1/app_documents?project_id=eq.${enc}&select=*&order=document_date.desc.nullslast,created_at.desc`),api(`/rest/v1/app_project_comments?project_id=eq.${enc}&select=*&order=created_at.desc`)]);return{p,modules:(modules||[]).filter(x=>x.enabled),workstreams:workstreams||[],progress:progress||[],milestones:milestones||[],tasks:tasks||[],docs:docs||[],memos:memos||[]}}
...
function progress(d){return `<section class="ps3-section" id="ps3-progress"><div class="ps3-section-head"><div><h3>${esc(moduleTitle(d,'progress','진행상황'))}</h3><p>사업의 현재 상태와 다음 할 일을 카드별로 관리합니다.</p></div><div class="ps3-actions"><button class="mini" data-ps3-add-ws>+ 진행상황 추가</button></div></div><div class="ps3-ws-grid">${d.workstreams.length?d.workstreams.map(ws=>{const updates=d.progress.filter(x=>x.workstream_id===ws.id),l=updates[0],phase=PHASE[ws.phase]||ws.phase||'진행';return `<article class="ps3-ws ps3-progress-card" data-ps3-progress-card="${ws.id}" data-phase="${esc(ws.phase||'in_progress')}"><div class="ps3-progress-card-head"><div class="ps3-progress-heading"><div class="ps3-progress-title-row"><h4>${esc(ws.title)}</h4><span class="ps3-progress-count">기록 ${updates.length}</span></div><div class="ps3-progress-meta"><span class="ps3-phase">${esc(phase)}</span>${l?.effective_on?`<span>최근 ${esc(date(l.effective_on))}</span>`:''}</div></div><button class="mini ps3-progress-edit" data-ps3-edit-ws="${ws.id}" aria-label="진행상황 항목 수정: ${esc(ws.title)}">항목 수정</button></div>${ws.description?`<p class="ps3-progress-description">${esc(ws.description)}</p>`:''}${l?`<div class="ps3-progress-current"><span class="ps3-progress-label">현재 상황</span><p>${esc(l.summary||'')}</p></div>${l.next_step?`<div class="ps3-progress-next"><span class="ps3-progress-label">다음 할 일</span><p>${esc(l.next_step)}</p></div>`:''}`:`<div class="ps3-progress-empty"><b>아직 기록이 없습니다.</b><span>첫 업데이트를 남겨 현재 상황을 기록하세요.</span></div>`}<button class="secondary ps3-update-btn" data-ps3-progress-ws="${ws.id}">진행상황 업데이트</button></article>`}).join(''):empty('진행상황이 없습니다.')}</div></section>`}
...
function memoHtml(d){return `<section class="ps3-section" id="ps3-memos"><div class="ps3-section-head"><div><h3>메모</h3><p>프로젝트 관련 메모를 간단히 누적합니다.</p></div></div><div class="ps3-memo-form"><textarea id="ps3MemoBody" rows="3" placeholder="메모를 입력하세요"></textarea><div class="ps3-memo-actions"><button class="mini hidden" type="button" data-ps3-memo-cancel>취소</button><button class="secondary" type="button" data-ps3-memo-save>메모 추가</button></div><div id="ps3MemoState" class="status"></div></div><div class="ps3-list ps3-memo-list">${d.memos.length?d.memos.map(x=>{const own=x.author_id===user?.id,del=own||canManage(d.p);return `<div class="ps3-row ps3-memo-row"><div class="ps3-row-main"><p>${esc(x.body)}</p><small>${esc(nameOf(x.author_id))} · ${fmt(x.updated_at||x.created_at)}</small></div><div class="ps3-memo-row-actions">${own?`<button class="mini" type="button" data-ps3-edit-memo="${x.id}">수정</button>`:''}${del?`<button class="mini ps3-danger" type="button" data-ps3-delete-memo="${x.id}">삭제</button>`:''}</div></div>`}).join(''):empty('메모가 없습니다.')}</div></section>`}
...
function renderDetail(d){const p=d.p,m=p.metadata||{},par=parent(p),ch=kids(p),manageable=canManage(p);$('#ps3Hierarchy').innerHTML=par?`<button class="ps3-parent-link" data-ps3-project="${par.id}">← ${esc(par.name)}</button>`:`${childMenu(ch,manageable)}`;$('#ps3Title').textContent=p.name;$('#ps3Objective').textContent=m.objective||p.description||'';const parts=[];if(enabled(d,'progress'))parts.push(progress(d));if(enabled(d,'milestones'))parts.push(milestones(d));if(enabled(d,'tasks'))parts.push(tasksHtml(d));if(enabled(d,'documents'))parts.push(docsHtml(d));parts.push(memoHtml(d));$('#ps3Body').innerHTML=`<div class="ps3-detail-head"><div class="ps3-actions">${manageable?(p.status==='done'?'<button class="mini" data-ps3-toggle-done>완료 취소</button>':'<button class="mini" data-ps3-toggle-done>완료</button>')+'<button class="mini" data-ps3-edit-project>수정</button><button class="mini" data-ps3-archive-project>보관</button><button class="mini ps3-danger" data-ps3-delete-project>삭제</button>':''}</div></div><nav class="ps3-nav">${MODULES.filter(([k])=>enabled(d,k)).map(([k,t])=>`<button data-ps3-nav="ps3-${k}">${t}</button>`).join('')}<button data-ps3-nav="ps3-memos">메모</button></nav>${parts.join('')}`;if(!manageable)$('#ps3Body .ps3-detail-head')?.remove();bindDocSearch()}
...
function resetProjectSession(){
  ...
  lastSpacesKey='';docFilter='all';docQuery='';
