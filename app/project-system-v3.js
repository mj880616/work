...
const countText=(c,id,docs=true)=>c?`할 일 ${c.tasks[id]||0}${docs?` · 자료 ${c.docs[id]||0}`:''}`:'';
function listRowHtml(p,ch,{open=true,c=listCounts,trail=''}={}){const exp=expandedTops.has(p.id)&&ch.length>0,kidsId=`ps3-kids-${esc(p.id)}`;
  const main=`<span class="ps3-prow-line1"><span class="ps3-prow-name">${esc(p.name)}</span>${p.status==='done'?'<span class="ps3-phase">완료</span>':''}</span><span class="ps3-prow-meta"><span data-ps3-count="${esc(p.id)}">${countText(c,p.id)}</span></span>`;
...
async function renderGrid(){
  ...
  const counts=await childCounts(listed);
  if(epoch!==projectEpoch)return;
  listCounts=counts;paintCounts(grid,counts)
}
