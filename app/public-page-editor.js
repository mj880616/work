(()=>{
  ...
  function ensureStyle(){
    ...
    s.textContent=`
      ...
      .ppe-block-delete:hover{background:#fff0f1;border-color:#c98288}
      .ppe-editing .pd-section>.ppe-block-delete{top:4px}
      @media(max-width:650px){.ppe-editing [data-ppe-editable]{outline-offset:2px}.ppe-live-status{display:block;flex:1 1 100%;order:5;margin:2px 0 0}.ppe-block-delete{right:4px;top:4px;padding:5px 7px}}
      @media print{.ppe-live-controls,.ppe-block-delete{display:none!important}.ppe-editing [data-ppe-editable]{outline:0!important;box-shadow:none!important}}
    `;
  ...
  function ensureTools(){
    ...
    if(!document.querySelector('#ppeLiveStatus')){
      ...
      retry.id='ppeLiveRetry';
      retry.type='button';
      retry.className='print-btn ppe-live-retry ppe-live-controls hidden';
      ...
      done.className='print-btn ppe-live-controls hidden';
      done.textContent='편집 종료';
  ...
  function showRetry(on){
    const el=document.querySelector('#ppeLiveRetry');
    if(el)el.classList.toggle('hidden',!on||!editing);
  }
  ...
  function addDeleteControl(target,label='칸'){
    if(!target||target.querySelector(':scope > .ppe-block-delete'))return;
    target.classList.add('ppe-deletable');
    const del=document.createElement('button');
    del.type='button';
    del.className='ppe-block-delete';
    del.dataset.ppeControl='1';
    del.contentEditable='false';
    del.setAttribute('aria-label',`${label} 전체 삭제`);
    del.textContent='삭제';
    del.addEventListener('click',e=>{
      e.preventDefault();
      e.stopPropagation();
      if(!editing)return;
      if(!window.confirm(`${label} 전체를 삭제할까요?`))return;
      target.remove();
      markDirty(true);
    });
  ...
  function bindInputTracking(paper){
    if(paper.dataset.ppeInputBound==='1')return;
    paper.dataset.ppeInputBound='1';
    paper.addEventListener('input',e=>{
      if(!editing)return;
      if(!e.target?.closest?.('[data-ppe-editable]'))return;
      markDirty(false);
    });
  }
  ...
  function prepareLiveFields(){
    ...
    paper.querySelectorAll('.pd-body .section-link').forEach(el=>{
      el.dataset.ppeControl='1';
  ...
  function inlineNode(node){
    ...
    if(el.dataset?.ppeControl==='1'||el.classList?.contains('ppe-block-delete'))return '';
    if(tag==='BR')return '\n';
  ...
  function serializeBlock(el,inSection=false){
    if(!el||el.dataset?.ppeControl==='1'||el.classList?.contains('ppe-block-delete'))return '';
    if(el.classList.contains('pd-forum-flow'))return serializeFlow(el);
    if(el.classList.contains('pd-section')){
      const children=[...el.children].filter(child=>child.dataset?.ppeControl!=='1'&&!child.classList?.contains('ppe-block-delete')),parts=[];
  ...
  function setSavingControls(on){
    ['#ppeLiveSave','#ppeLiveDone','#ppeLiveCancel'].forEach(selector=>{
  ...
  function scheduleAutosave(){
    clearAutosave();
    if(!editing||dirtyVersion<=savedVersion)return;
    autosaveTimer=setTimeout(()=>{
      autosaveTimer=null;
      void persistDirty({autoRetry:true});
    },AUTOSAVE_DELAY);
  }
  ...
  function markDirty(immediate=false){
    ...
    clearRetry();
    showRetry(false);
    setStatus(immediate?'저장 중…':'저장 대기 중…');
    if(immediate){
      clearAutosave();
      void persistDirty({autoRetry:true});
  ...
  function scheduleRetry(error){
    ...
    if(retryAttempt<RETRY_DELAYS.length){
      ...
      setStatus(`저장 실패 · ${Math.ceil(delay/100)/10}초 후 다시 시도`,true);
    ...
    showRetry(true);
    setStatus(`${error?.message||'저장 실패'} · 수정 내용은 화면에 유지됨`,true);
  }

  async function persistDirty({autoRetry=true}={}){
    clearAutosave();
    if(!editing||!current?.id)return true;
    if(saving)return savePromise||false;
  ...
  async function retrySave(){
    ...
    retryAttempt=0;
    showRetry(false);
    await persistDirty({autoRetry:true});
  }
  ...
  async function finishEditing(){
    ...
    clearRetry();
    let ok=true;
    if(saving)ok=await (savePromise||Promise.resolve(false));
    if(ok&&dirtyVersion>savedVersion)ok=await persistDirty({autoRetry:false});
    if(!ok||dirtyVersion>savedVersion)return;
    exitEditing();
  }
  ...
  function setEditingUi(on){
    ...
    const saveBtn=document.querySelector('#ppeLiveSave'),done=document.querySelector('#ppeLiveDone'),cancel=document.querySelector('#ppeLiveCancel'),status=document.querySelector('#ppeLiveStatus');
    edit?.classList.toggle('hidden',on||secureMode||!current?.id||!canEdit);
    print?.classList.toggle('hidden',on);
    [saveBtn,done,cancel,status].forEach(x=>x?.classList.toggle('hidden',!on));
    if(!on)showRetry(false);
    if(on)setStatus('수정 중 · 자동 저장');
  }
  ...
  function cancelEdit(){
    ...
    const hasUnsaved=dirtyVersion>savedVersion;
    if(hasUnsaved&&!window.confirm('아직 저장되지 않은 변경을 버릴까요?'))return;
    exitEditing();
  }
  ...
  window.addEventListener('beforeunload',e=>{
    if(!editing||(!saving&&dirtyVersion<=savedVersion))return;
    e.preventDefault();
    e.returnValue='';
  });
