(()=>{
  ...
  async function runtime(){
    ...
    if(!window.KPTURuntime?.api||!window.KPTURuntime?.session)throw new Error('편집 인증 모듈을 초기화하지 못했습니다. 새로고침 후 다시 시도해 주세요.');
  ...
  function prepareLiveFields(){
    const paper=document.querySelector('#paper');if(!paper)return false;
    const title=paper.querySelector('.pd-title');let summary=paper.querySelector('.pd-summary');const hero=paper.querySelector('.pd-hero');
    ...
    paper.querySelectorAll('.pd-forum-flow').forEach(prepareFlow);return true;
  }
  function inlineNode(node){
    if(node.nodeType===Node.TEXT_NODE)return String(node.nodeValue||'').replace(/\u00a0/g,' ');
    if(node.nodeType!==Node.ELEMENT_NODE)return '';
    const el=node,tag=el.tagName;if(tag==='BR')return '\n';const inner=[...el.childNodes].map(inlineNode).join('');
    if(tag==='STRONG'||tag==='B')return '**'+inner+'**';if(tag==='EM'||tag==='I')return '*'+inner+'*';return inner;
  }
  function inlineText(el){return [...(el?.childNodes||[])].map(inlineNode).join('').replace(/[ \t]+\n/g,'\n').trim()}
  function serializeFlow(flow){
    const top=[...flow.querySelectorAll('.pd-flow-top .pd-flow-node')].map(cleanText).filter(Boolean);const tracks=[...flow.querySelectorAll('.pd-flow-track')];
    const readTrack=track=>{const problem=track?.querySelector('.pd-flow-problem');const field=problem?.querySelector(':scope > [data-ppe-flow-problem]');return{problem:cleanText(field||problem),note:cleanText(problem?.querySelector(':scope > .pd-flow-note')),solution:cleanText(track?.querySelector('.pd-flow-solution'))}};
  ...
  function serializeBlock(el,inSection=false){
    if(!el)return '';if(el.classList.contains('pd-forum-flow'))return serializeFlow(el);
    if(el.classList.contains('pd-section')){const children=[...el.children],parts=[];children.forEach((child,i)=>{if(i===0&&child.tagName==='H2')parts.push('## '+inlineText(child));else{const v=serializeBlock(child,true);if(v)parts.push(v)}});return parts.join('\n\n')}
  ...
  function collectLivePage(){const paper=document.querySelector('#paper'),title=cleanText(paper?.querySelector('.pd-title')),summary=cleanText(paper?.querySelector('.pd-summary')),bodyEl=paper?.querySelector('.pd-body'),body=bodyEl?serializeChildren(bodyEl,false):'';return{title,summary,body}}
  function setEditingUi(on){
    editing=on;document.body.classList.toggle('ppe-editing',on);const edit=btn(),print=document.querySelector('#printPageBtn');const saveBtn=document.querySelector('#ppeLiveSave'),cancel=document.querySelector('#ppeLiveCancel'),status=document.querySelector('#ppeLiveStatus');
    edit?.classList.toggle('hidden',on||secureMode||!current?.id);print?.classList.toggle('hidden',on);[saveBtn,cancel,status].forEach(x=>x?.classList.toggle('hidden',!on));if(on)setStatus('수정 중 · 바꾸고 싶은 문구를 화면에서 직접 클릭');
  }
  async function open(){
    if(secureMode||!current?.id||editing)return;
    try{await checkAccess();ensureTools();setEditingUi(true);if(!prepareLiveFields())throw new Error('편집할 내용을 찾지 못했습니다.');document.querySelector('#paper .pd-title')?.focus()}
    catch(e){setEditingUi(false);alert(e?.message||'수정 권한을 확인하지 못했습니다.')}
  }
  function cancelEdit(){if(saving||!editing)return;editing=false;document.body.classList.remove('ppe-editing');if(typeof window.KPTURenderPublicPage==='function')window.KPTURenderPublicPage(current,false);setEditingUi(false)}
  async function save(){
    if(saving||!editing||!current?.id)return;const next=collectLivePage();if(!next.title){setStatus('제목은 비워둘 수 없습니다.',true);return}
    const saveBtn=document.querySelector('#ppeLiveSave');saving=true;if(saveBtn){saveBtn.disabled=true;saveBtn.textContent='저장 중…'}setStatus('저장 중…');
    try{const d=await updatePage(next);if(!d?.page)throw new Error(d?.error||'저장 결과를 확인하지 못했습니다.');current={...current,...d.page};editing=false;document.body.classList.remove('ppe-editing');if(typeof window.KPTURenderPublicPage==='function')window.KPTURenderPublicPage(current,false);setEditingUi(false)}
    catch(e){setStatus(e?.message||'저장에 실패했습니다. 수정 내용은 화면에 유지됩니다.',true)}finally{saving=false;if(saveBtn){saveBtn.disabled=false;saveBtn.textContent='저장'}}
  }
  function setPage(page,secure=false){current=page||null;secureMode=!!secure;ensureTools();const b=btn();if(!b)return;b.onclick=null;if(secureMode||!current?.id){b.classList.add('hidden');return}if(!editing)b.classList.remove('hidden');b.onclick=e=>{e.preventDefault();open()}}
  document.addEventListener('keydown',e=>{if(!editing)return;if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='s'){e.preventDefault();save();return}if(e.key==='Escape'){e.preventDefault();cancelEdit()}});
  ensureStyle();window.KPTUPublicPageEditor={setPage,open,cancel:cancelEdit,save};
})();
