...
async function render(){if(renderingEpoch===sessionEpoch||!userId||!workspaceId)return;const r=root();if(!r)return;rememberOpen();const owner=userId,workspace=workspaceId,epoch=sessionEpoch;renderingEpoch=epoch;try{const rows=await api(`/rest/v1/app_tasks?workspace_id=eq.${encodeURIComponent(workspace)}&assignee_id=eq.${encodeURIComponent(owner)}&select=*&order=due_at.asc.nullslast,created_at.desc`)||[];if(epoch!==sessionEpoch||owner!==userId||workspace!==workspaceId)return;tasks=rows;const mine=tasks.filter(t=>!teamAssigned(t)),assigned=tasks.filter(teamAssigned);r.innerHTML=group('mine','내 할 일',mine)+group('assigned','팀에서 부여된 할 일',assigned)}catch(e){if(epoch===sessionEpoch)r.innerHTML=`<div class="status error">${esc(e.message||String(e))}</div>`}finally{if(renderingEpoch===epoch)renderingEpoch=-1}}
