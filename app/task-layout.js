...
function card(t){return window.KPTUTaskRowView.render({...t,dueLabel:due(t.due_at)},{scope:'tl',context:projectName(t.project_id),origin:origin(t),canDelete:canDelete(t)})}
...
function closeModal({restoreFocus=true}={}){const m=$('#taskModal');if(m){m.classList.add('hidden');m.setAttribute('aria-hidden','true');window.KPTUA11y?.dialog.deactivate(m,{restoreFocus,fallbackFocus:'#newTaskBtn'})}setMode(null)}
...
function install(){
  ...
  document.addEventListener('click',e=>{
    if(e.target.closest?.('#quickTaskBtn,#newTaskBtn')){setMode(null);return}
