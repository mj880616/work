...
function group(key,title,rows){const pending=rows.filter(t=>t.status!=='done'),done=rows.filter(t=>t.status==='done'),pendingKey=`${key}:pending`,doneKey=`${key}:done`;return `<section class="tl-task-section" data-tl-section="${key}"><div class="tl-section-head"><h3>${esc(title)}</h3><span>${pending.length}건</span></div><details class="tl-status-group tl-incomplete" data-tl-state="${pendingKey}" ${stateOpen(pendingKey,true)?'open':''}><summary><span>미완료된 할 일</span><b>${pending.length}</b></summary><div class="tl-open-list">${pending.length?pending.map(card).join(''):'<div class="empty compact">미완료된 할 일이 없습니다.</div>'}</div></details><details class="tl-status-group tl-completed" data-tl-state="${doneKey}" ${stateOpen(doneKey,false)?'open':''}><summary><span>완료된 할 일</span><b>${done.length}</b></summary><div class="tl-done-list">${done.length?done.map(card).join(''):'<div class="empty compact">완료된 할 일이 없습니다.</div>'}</div></details></section>`}
async function context(){const epoch=sessionEpoch;if(!(await rt.session.ensure())||epoch!==sessionEpoch)return false;const shared=rt.context?.read?.()||window.__KPTU_BOOT_CONTEXT__;if(shared?.user?.id&&shared?.workspace?.id){userId=shared.user.id;workspaceId=shared.workspace.id;await loadProjects();return epoch===sessionEpoch}const u=await api('/auth/v1/user');if(epoch!==sessionEpoch)return false;const ms=await api(`/rest/v1/app_workspace_members?user_id=eq.${encodeURIComponent(u.id)}&select=workspace_id&limit=1`);if(epoch!==sessionEpoch||!ms?.length)return false;userId=u.id;workspaceId=ms[0].workspace_id;await loadProjects();return epoch===sessionEpoch}
async function loadProjects(){if(!workspaceId||!userId)return;const workspace=workspaceId,owner=userId,epoch=sessionEpoch;const rows=await api(`/rest/v1/app_spaces?workspace_id=eq.${encodeURIComponent(workspace)}&owner_id=eq.${encodeURIComponent(owner)}&status=neq.archived&select=id,name,parent_id,status,metadata&order=sort_order.asc,created_at.asc`);if(epoch!==sessionEpoch||workspace!==workspaceId||owner!==userId)return;const active=rows||[],included=new Set(active.filter(x=>x.metadata?.project_system==='v2'||Number(x.metadata?.management_version)===2).map(x=>x.id));let changed=true;while(changed){changed=false;for(const x of active)if(x.parent_id&&included.has(x.parent_id)&&!included.has(x.id)){included.add(x.id);changed=true}}projects=active.filter(x=>included.has(x.id))}
...
function projectOptions(selected=''){return '<option value="">프로젝트 없음</option>'+projects.filter(x=>x.status!=='archived').map(x=>`<option value="${esc(x.id)}" ${x.id===selected?'selected':''}>${esc(x.parent_id?'↳ ':'')}${esc(x.name)}</option>`).join('')}
function openCreate(projectId=''){
  setMode(null);
  const vals={taskTitle:'',taskDue:'',taskDescription:'',taskNote:''};
  Object.entries(vals).forEach(([id,value])=>{const el=$('#'+id);if(el)el.value=value});
  if($('#taskPriority'))$('#taskPriority').value='normal';
  const select=$('#taskProject');
  if(select){select.innerHTML=projectOptions(projectId);if(projectId){ensureProjectOption(projectId);select.value=projectId}}
  const modal=$('#taskModal');
  if(modal){
    modal.classList.remove('hidden');
    modal.classList.toggle('from-project',Boolean(projectId));
    modal.setAttribute('aria-hidden','false');
    window.KPTUA11y?.dialog.activate(modal,{trigger:document.activeElement,initialFocus:'#taskTitle',onRequestClose:closeModal});
  }
  $('#taskTitle')?.focus()
}
function openEdit(id,focus='title'){const t=tasks.find(x=>x.id===id);if(!t)return;setMode(t);ensureProjectOption(t.project_id);const vals={taskTitle:t.title||'',taskProject:t.project_id||'',taskDue:dateValue(t.due_at),taskPriority:t.priority||'normal',taskDescription:t.description||'',taskNote:t.note||''};Object.entries(vals).forEach(([id,v])=>{const e=$('#'+id);if(e)e.value=v});const m=$('#taskModal');if(m){m.classList.remove('hidden');m.classList.toggle('from-project',!$('#ps3DetailModal')?.classList.contains('hidden'));m.setAttribute('aria-hidden','false');window.KPTUA11y?.dialog.activate(m,{trigger:document.activeElement,initialFocus:focus==='note'?'#taskNote':'#taskTitle',onRequestClose:closeModal})}$('#'+(focus==='note'?'taskNote':'taskTitle'))?.focus()}
...
window.KPTUTaskLayout={openTask,openCreate};
