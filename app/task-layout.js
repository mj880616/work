...
async function loadProjects(){if(!workspaceId||!userId)return;const workspace=workspaceId,owner=userId,epoch=sessionEpoch;const rows=await api(`/rest/v1/app_spaces?workspace_id=eq.${encodeURIComponent(workspace)}&owner_id=eq.${encodeURIComponent(owner)}&status=neq.archived&select=id,name,parent_id,status&order=sort_order.asc,created_at.asc`);if(epoch!==sessionEpoch||workspace!==workspaceId||owner!==userId)return;projects=rows||[]}
