...
let tavUser=null,tavRows=[],tavBusy=false,tavTimer=null;
...
function tavApply(){
  ...
  if(secs.length>=2){
    const mine=tavRows.filter(t=>(t.source_type||'manual')==='manual'&&t.created_by===tavUser);
    const assigned=tavRows.filter(t=>!((t.source_type||'manual')==='manual'&&t.created_by===tavUser));
    const a=secs[0].querySelector('.tl-section-head span'),b=secs[1].querySelector('.tl-section-head span');
    if(a)a.textContent=mine.filter(t=>t.status!=='done').length+'건';
    if(b)b.textContent=assigned.filter(t=>t.status!=='done').length+'건';
  }
...
async function tavRefresh(){
  if(tavBusy)return;tavBusy=true;
  try{
    const s=tavSession();if(!s?.access_token)return;
    if(!tavUser){
      const r=await fetch(TAV_SB+'/auth/v1/user',{headers:{apikey:TAV_KEY,Authorization:'Bearer '+s.access_token},cache:'no-store'});
      if(!r.ok)return;tavUser=(await r.json()).id;
    }
    tavRows=await tavApi('/rest/v1/app_tasks?assignee_id=eq.'+tavUser+'&select=id,status,source_type,created_by');
    tavApply();
  }finally{tavBusy=false}
}
