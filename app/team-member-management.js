...
function tmmRefreshRoleText(role){const el=document.querySelector('#tpvTitle');if(!el)return;const parts=(el.textContent||'').split(' · ').filter(Boolean);if(!parts.length){el.textContent=TMM_ROLE[role]||role;return}const last=parts[parts.length-1];if(Object.values(TMM_ROLE).includes(last))parts[parts.length-1]=TMM_ROLE[role]||role;else parts.push(TMM_ROLE[role]||role);el.textContent=parts.join(' · ')}
function tmmNotify(detail){window.dispatchEvent(new CustomEvent('kptu:team-members-changed',{detail}));window.dispatchEvent(new CustomEvent('kptu:view-changed',{detail:{view:'team'}}))}
function tmmLimitApprovalRoles(){
  document.querySelectorAll('[data-aa-role],#inviteRole').forEach(sel=>{sel.querySelector('option[value="admin"]')?.remove()});
  const head=document.querySelector('#aaReviewSection .aa-review-head>div');
  if(head&&!head.querySelector('.tmm-approval-note'))head.insertAdjacentHTML('beforeend','<small class="tmm-approval-note">관리자 권한은 가입 승인 후 소유자가 구성원 상세에서 지정합니다.</small>');
  const invite=document.querySelector('#inviteRole');
  if(invite&&!invite.parentElement?.querySelector('.tmm-approval-note'))invite.insertAdjacentHTML('afterend','<small class="tmm-approval-note">관리자 권한은 가입 후 구성원 상세에서 지정합니다.</small>');
}
...
async function tmmSave(target){const select=document.querySelector('#tmmRole'),status=document.querySelector('#tmmStatus');if(!select)return;const role=select.value;if(role===target.role){status.textContent='변경된 권한이 없습니다.';return}if(!confirm(`${TMM_ROLE[target.role]||target.role} → ${TMM_ROLE[role]||role} 권한으로 변경할까요?`))return;status.textContent='변경 중…';status.className='tmm-status';try{await tmmRpc('app_set_workspace_member_role',{p_user:target.user_id,p_role:role});status.textContent='권한을 변경했습니다.';status.className='tmm-status success';tmmRefreshRoleText(role);tmmNotify({user_id:target.user_id,role});setTimeout(()=>tmmRender(),80)}catch(e){status.textContent=e.message||String(e);status.className='tmm-status error'}}

async function tmmRemove(target){const name=document.querySelector('#tpvName')?.textContent||'이 구성원';if(!confirm(`${name} 님을 팀에서 제외할까요?\n\nWorkspace 접근 권한이 즉시 해제됩니다.`))return;const status=document.querySelector('#tmmStatus');status.textContent='처리 중…';status.className='tmm-status';try{await Promise.allSettled([tmmApi(`/rest/v1/app_suborganization_assignees?user_id=eq.${target.user_id}`,{method:'DELETE'}),tmmApi(`/rest/v1/app_profile_workplaces?user_id=eq.${target.user_id}`,{method:'DELETE'})]);await tmmApi(`/rest/v1/app_workspace_members?workspace_id=eq.${tmmWorkspace}&user_id=eq.${target.user_id}`,{method:'DELETE'});document.querySelector('#tpvClose')?.click();tmmNotify({user_id:target.user_id,removed:true});window.dispatchEvent(new CustomEvent('kptu:suborganization-updated'))}catch(e){status.textContent=e.message||String(e);status.className='tmm-status error'}}
...
function tmmBind(){document.addEventListener('click',e=>{const b=e.target.closest?.('[data-tpv-open]');if(b?.dataset.tpvOpen)tmmSchedule(b.dataset.tpvOpen,180)},true);const body=document.querySelector('#tpvBody');if(body)new MutationObserver(()=>{if(tmmTarget&&!document.querySelector('#tmmSection'))tmmSchedule(null,30)}).observe(body,{childList:true,subtree:false});const team=document.querySelector('#teamView');if(team)new MutationObserver(()=>tmmLimitApprovalRoles()).observe(team,{childList:true,subtree:true});const invite=document.querySelector('#inviteModal');if(invite)new MutationObserver(()=>tmmLimitApprovalRoles()).observe(invite,{childList:true,subtree:true});window.addEventListener('kptu:team-members-changed',()=>tmmSchedule(null,40));window.addEventListener('kptu:view-changed',e=>{if(e.detail?.view==='team')setTimeout(tmmLimitApprovalRoles,100)})}

(async()=>{if(!(await tmmContext()))return;tmmStyle();tmmBind();tmmLimitApprovalRoles()})().catch(console.error);
