(()=>{
  ...
  function renderMarkdown(src=''){
    ...
    const closeDetails=()=>{
      closeList();
      if(details){out+='</div></details>';details=false;}
    };
    const closeSection=()=>{
      closeDetails();
      if(section){out+='</section>';section=false;}
    };
    ...
    for(const raw of lines){
      ...
      if((m=line.match(/^##\s+(.+)$/))){
        closeSection();
        out+=`<section class="pd-section"><h2>${inline(m[1])}</h2>`;
        section=true;
        continue;
      }

      if((m=line.match(/^:::details\s+(.+)$/))){
        closeDetails();
        out+=`<details class="pd-details"><summary>${inline(m[1])}</summary><div class="pd-details-body">`;
        details=true;
        continue;
      }
      ...
      if((m=line.match(/^[-*]\s+\[([ xX])\]\s+(.+)$/))){
        if(list!=='ul'){closeList();list='ul';out+='<ul class="pd-checklist">';}
        const done=m[1].toLowerCase()==='x';
        out+=`<li class="pd-check ${done?'pd-check-done':'pd-check-open'}"><span class="pd-checkbox" aria-hidden="true">${done?'✓':''}</span><span>${inline(m[2])}</span></li>`;
        continue;
      }
      ...
      if((m=line.match(/^\d+[.)]\s+(.+)$/))){
        if(list!=='ol'){closeList();list='ol';out+='<ol>';}
        out+=`<li>${inline(m[1])}</li>`;
        continue;
      }
  ...
  function ensureStyles(){
    ...
    s.textContent=`
      ...
      .pd-layout-checklist .pd-details{margin:10px 0;border:1px solid #dde5eb;border-radius:14px;background:#fff;overflow:hidden;box-shadow:0 2px 8px rgba(23,50,77,.035)}
      .pd-layout-checklist .pd-details summary{list-style:none;cursor:pointer;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 16px;font-weight:900;color:var(--pd-accent);user-select:none}
      .pd-layout-checklist .pd-details summary::-webkit-details-marker{display:none}
      .pd-layout-checklist .pd-details summary::after{content:'＋';flex:0 0 auto;font-size:18px;line-height:1;color:#7a8995}
      .pd-layout-checklist .pd-details[open] summary{background:#fbfcfd;border-bottom:1px solid #e5ebef}
      .pd-layout-checklist .pd-details[open] summary::after{content:'−'}
      .pd-layout-checklist .pd-details-body{padding:5px 16px 16px}
      .pd-layout-checklist .pd-details-body h4{font-size:15px;color:var(--pd-accent);font-weight:900;margin:16px 0 6px}
      .pd-layout-checklist .pd-details-body p{color:#53616e}

      @media(max-width:700px){.pd-hero-split .pd-hero{grid-template-columns:1fr;gap:10px}.pd-hero-split .pd-summary{padding-left:0;border-left:0}.pd-sections-cards .pd-body{grid-template-columns:1fr}.pd-design .pd-hero-band .pd-hero{padding:18px}.pd-layout-checklist .pd-hero{padding:22px 19px!important;border-radius:17px}.pd-layout-checklist .pd-title{font-size:27px}.pd-layout-checklist .pd-body>.pd-section{padding:15px 14px!important;border-radius:14px!important}.pd-layout-checklist .pd-check{padding:9px 10px}.pd-layout-checklist .pd-details summary{padding:13px 14px}.pd-layout-checklist .pd-details-body{padding:4px 14px 14px}}
    `;
