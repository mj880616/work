...
| 이름-1 | 앱 내부 "공공기관사업팀 Workspace" 이름을 "웹2"로 교체 | 진행중 | #316 | 기준 `99512a1`. UI 문구만(DB·Edge 변경 없음). app/windows-manifest.json name·short_name, app/brand-logo.js 로고 대체 글자, app/legacy/startup-speed.js 기본값, app/ARCHITECTURE.md 제목, tests/app-e2e 모의 workspace 이름. 제외: workspace/index.html·workspace/privacy/index.html(구글 등록 정보와 별도 작업), supabase/functions 드라이브 폴더 이름, 원장 과거 기록. 캐시: index.html의 `windows-manifest.json?v=5`→`v6`. brand-logo.js·legacy/startup-speed.js는 현재 어느 로더도 불러오지 않아 올릴 로더 버전 없음 |
