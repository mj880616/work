...
- 2026-09-26 일정 탭: 유지하고 "이번 주 업무" 화면으로 전환한다. 보기 전환 [목록|주간|월간], 기존 월간 달력은 유지한다. 기본 보기는 접힌 화면(폴더블 접힘·좁은 휴대폰)=목록, 펼친 화면·PC=주간. 캘린더 기본 표시는 공공운수노조 캘린더이고, 전체 캘린더를 켤 수 있다. 목록에는 이번 주 마감 Google Tasks와 회의를 포함한다. 디자인 전면 개선에 포함한다.

## 작업 표

완료·종료 행 아래의 진행중·대기 행은 위에서부터 순서대로 진행한다. DB-1·DB-2는 비고의 작업에서 함께 처리하고, ENV-4는 수시로 진행한다. PR 칸은 `git log` 또는 GitHub PR 목록으로 확인된 번호만 적는다. 확인되지 않은 칸은 비워 둔다.
...
| 디자인 | 디자인 전면 개선(D-1~D-6: 전수검사→기준→공통부품→화면적용→넓은화면 목록+상세→덧칠정리·자동검사) | 대기 | | 17b-2(17b 나머지 목록 정리) 통합. 일정 탭 "이번 주 업무" 화면 전환 포함(보기 전환 [목록|주간|월간], 기본 보기 접힌=목록·펼친/PC=주간, 기본 캘린더 공공운수노조·전체 켜기 가능, 목록에 이번 주 마감 Google Tasks·회의. 결정사항 참고) |
