...
| ENV-7 | Codex 병행 준비 | 진행중 | #323 | 기준 `a3666fb`. 사용자 설치·시험 후 완료. 이 PR: AGENTS.md를 유일한 규칙 원문으로 통합(운영 규칙·도구 4종 구분·캐시 버전 규칙 명문화·사용자 할 일과 결정 항목 PR 댓글 게시), CLAUDE.md는 `@AGENTS.md` 불러오기 + Claude Code 전용 3줄, migration-history.md 참조 수정. verify_jwt 사실 정정: production 30개 중 true 8·false 22, 함수별 표는 [ENV-6b 문서](web2-env6b-edge-source.md) 5절. Codex CLI(Supabase MCP 조회 전용·이 프로젝트 한정)·Codex 웹 연결은 저장소 밖 사용자 단계(비밀값 저장소·Codex 웹 환경에 넣지 않음). 남은 일: 사용자 설치·연결, Codex로 조회 작업 1건 시험 |
