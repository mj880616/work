...
| SEC-1 | auth-handoff 보안 점검 | 진행중 | | 배포 대기. 기준 `8a6f4ec5`(#323). [조사·정지 지점](web2-sec1-auth-handoff.md). 1회 사용 보장에 DB 소비 기록 필요 → 사용자 지시 2(a)에 따라 SQL 초안에서 정지. 함수·앱 코드 수정, DB 적용, Edge 배포 미실행. 검토용 PR에서 조사·재현·SQL 초안·배포 준비만 정리 |
