...
- Supabase URL은 `https://xmlkxfjeagycwttklxjw.supabase.co`이며 Web1/Web2 및 읽생기 클라이언트에서 직접 사용함. 변경 전 Supabase Auth Site URL과 유일한 Redirect URL은 `https://mj880616.github.io/work/app/`였음. Site URL은 유지하고 Redirect URLs만 아래 범위로 추가함. Web2 비밀번호 복구와 가입 확인의 기존 고정 주소는 새 호스트에서 동적 주소로 변경함.
...
Supabase Auth Site URL은 기존 github.io 주소로 유지함. 운영 Redirect URLs에는 `desk`·`work`의 `/work/app/`와 `/work/app/confirmed.html`, 읽생기의 새·기존 `/read-think-write/`를 경로 단위로 등록함. Web2 Google 로그인은 `view`, `invite`, `project` 쿼리로 현재 화면을 전달하므로 해당 앱 루트의 쿼리 패턴만 허용함. Web1 관리자 로그인은 정확한 페이지 경로로 복귀하고, 복귀 전 쿼리는 같은 탭의 sessionStorage에서 복원함. Web1 편집 페이지는 `/work/2in1/`, `/work/p/`, `/work/private-rail/`, `/work/rail-council/`, `/work/review/`, `/work/sanbyeol/`, `/work/workforce/`의 실제 경로 깊이에 한정한 단일 구간 `*` 패턴으로 허용함. 루트 포털과 아스날 아카이브는 인증 복귀 주소로 등록하지 않음.
