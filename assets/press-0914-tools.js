...
function installStyle(){
  ...
  .file-list{margin-top:12px;border-top:1px solid #edf0f2}.file-row{display:grid;grid-template-columns:50px minmax(0,1fr);gap:10px;padding:10px 0;border-bottom:1px solid #edf0f2;align-items:center}.file-badge{display:flex;align-items:center;justify-content:center;min-height:31px;border-radius:8px;background:#eef1f4;color:#59626d;font-size:10px;font-weight:900;letter-spacing:.2px}.file-name{font-size:13px;font-weight:800;overflow-wrap:anywhere}.file-meta{font-size:11px;color:#7a828c;margin-top:3px}.file-empty{font-size:12px;color:#7a828c;padding:13px 0}.file-head{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-top:14px}.file-head b{font-size:13px}.file-refresh{border:0;background:transparent;color:#65717d;font:inherit;font-size:11px;font-weight:800;cursor:pointer;padding:4px}
  @media(max-width:620px){.file-upload-grid{grid-template-columns:1fr}.file-upload-btn{width:100%}.file-row{grid-template-columns:44px minmax(0,1fr)}}`;
...
function injectFiles(){
  ...
  sec.innerHTML=`<h2>관련 파일</h2><p class="desc">보도자료·발언문·현장사진 등 기자회견 관련 파일을 올립니다. 별도 로그인 없이 업로드할 수 있습니다.</p><div class="file-upload-box"><div class="file-upload-grid"><div class="file-field"><label for="relatedFile">파일 선택</label><input id="relatedFile" type="file"></div><div class="file-field"><label for="fileUploader">이름·조직 <span style="font-weight:500;color:#8a929b">(선택)</span></label><input id="fileUploader" type="text" maxlength="80" placeholder="예: 철도노조 홍길동"></div><button type="button" class="file-upload-btn" id="fileUploadBtn">파일 업로드</button></div><input class="file-honeypot" id="fileWebsite" type="text" tabindex="-1" autocomplete="off" aria-hidden="true"><div class="file-status" id="fileUploadStatus"></div><div class="file-note">※ 파일당 최대 30MB · 실행파일 등 위험한 형식은 업로드되지 않음.</div></div><div class="file-head"><b>최근 업로드</b><button type="button" class="file-refresh" id="fileRefreshBtn">새로고침</button></div><div class="file-list" id="relatedFileList"><div class="file-empty">파일 목록을 불러오는 중…</div></div>`;
  ...
  async function load(){list.innerHTML='<div class="file-empty">파일 목록을 불러오는 중…</div>';try{const data=await request('list'),files=Array.isArray(data.files)?data.files:[];if(!files.length){list.innerHTML='<div class="file-empty">아직 업로드된 파일이 없습니다.</div>';return}list.innerHTML=files.map(f=>{const meta=[f.size_text||'',fileDate(f.created_at),f.uploader?`업로드: ${f.uploader}`:''].filter(Boolean).join(' · ');return `<div class="file-row"><div class="file-badge">${typeLabel(f.name)}</div><div><div class="file-name">${escapeHtml(f.name||'파일')}</div><div class="file-meta">${escapeHtml(meta)}</div></div></div>`}).join('')}catch(e){list.innerHTML=`<div class="file-empty">목록을 불러오지 못했습니다. ${escapeHtml(e.message||'')}</div>`}}
